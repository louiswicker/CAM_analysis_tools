...
! </table>
   ...
   use fv_mapz_mod        only: cs_profile
#ifdef MULTI_GASES
...
! Excluding contribution from condensates:
      ...
      elseif ( a_imp <= 0.5 ) then
           ...
           call SIMC_solver(dt, is1, ie1, km, rdgas, gama, gm2, cp2, akap, &
#ifdef MULTI_GASES
                            kapad2, &
#endif
                            pe2,  &
...
! peln used during remap; pk3 used only for p_grad
      ...
      elseif ( a_imp > 0.999 ) then
           call SIMC_solver(dt, is, ie, km, rdgas, gama, gm2, cp2, akap, &
#ifdef MULTI_GASES
                            kapad2, &
#endif
                            pe2, dm,   &
...
#endif
! Local
   real, dimension(is:ie,km  ):: aa, bb, dd, w1, g_rat, gam,dz2_save
...
! Recompute p' at center of the zones.
    ...
    IF( (is .lt. 128) .and. (ie .gt. 128) .and. (jslice .eq. 128)) then
       ...
       do k = 1,km
          write(66,*), k, pe(128,k), w2(128,k), (dz2(128,k)-dz2_save(128,k))/dt
       enddo

       write(66,*), km+1, pe(128,km+1)
...
#endif
! Local
   real, dimension(is:ie,km  ):: aa, bb, dd, w1, g_rat, gam,dz2_save, pp, dm2_tmp
   ...
   integer i, k
    ...
    do k=1,km
       ...
       enddo
    enddo
!At this point pe is pert p valid at the grid centers, and pm2 is full pressure at grid centers

! Use cubic spline routine to map w from grid centers (w2 at pm2) to grid edges (we at pem)

 call cs_profile( ws, q4, dm2, km, is, ie, -2, kord_wz )
...
! Solve for p' at grid centers (pe) using w at grid edges (we)

 do k = 1,km
   do i=is,ie
     pe(i,k) = pe(i,k) - gm2(i,k)*pm2(i,k)*(we(i,k+1)-we(i,k))/dz2(k)*dt 
     dm2_tmp(i,k) = pm2(i,k) + pe(i,k)
   enddo
 enddo
 
 
 ! Set up tridiagonal coeffs for cubic spline interpolation of p' back to grid edges

    do k=1,km-1
       do i=is, ie
          g_rat(i,k) = dm2_tmp(i,k)/dm2_tmp(i,k+1)
             bb(i,k) = 2.*(1.+g_rat(i,k))
             dd(i,k) = 3.*(pe(i,k) + g_rat(i,k)*pe(i,k+1))
       enddo
    enddo
...
#endif
       z_new(i,km+1) = 0.0
       z_new(i,km) = z(i,km+1) + dzs
    enddo

    do k=km-1, 1, -1
       do i=is, ie
          z_old(i,k) =  z_old(i,k-1) + dz2(i,k)
          p1(i) = pe(i,k)
#ifdef MOIST_CAPPA
          dz2(i,k) = -dm2(i,k)*rgas*pt2(i,k)*exp((cp2(i,k)-1.)*log(max(p_fac*pm2(i,k),p1(i)+pm2(i,k))))
...
#endif
#endif
          z_new(i,k) =  z_new(i,k-1) + dz2(i,k)
       enddo
    enddo
    
! Update w at grid edges (w2) using new dz2 (dz/dt)
! w = dz/dt
    do k = 1,km
      do i = is, ie
        w2(i,k) = (z_new(i,k)-z_old(i,k))/dt
      enddo
    enddo
