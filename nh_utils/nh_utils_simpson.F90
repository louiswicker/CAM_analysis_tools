...
! 
       do i=is, ie
!         pe(i,km) = pe(i,km-1) + dm2(i,km-1)*(w2(i,km-1)-w1(i,km-1))*rdt
         pe(i,km+1) = pe(i,km) + dm2(i,km)*(w2(i,km)-w1(i,km))*rdt
       enddo
!
!       do k=1,km
!        do i=is, ie
!          pptest(i,k+1) = pptest(i,k) + dm2(i,k)*(w2(i,k)-w1(i,k))*rdt
!        enddo
!      enddo
...
!    do k=1,km-1
!       do i=is, ie
!          aa(i,k+1) = dt*(gm2(i,k+1)+gm2(i,k))/(dz2(i,k+1)+dz2(i,k)) * (pem(i,k+1)+pp(i,k+1))
!          pe(i,k+1) = pp(i,k+1) - aa(i,k+1)*(w2(i,k+1)-w2(i,k))
!          !update full pressure again
!          !aa(i,k+1) = dt*(gm2(i,k+1)+gm2(i,k))/(dz2(i,k+1)+dz2(i,k)) *(pem(i,k+1)+pe(i,k+1))
!          !pe(i,k+1) = pe(i,k+1) - aa(i,k+1)*(w2(i,k+1)-w2(i,k))/dt
!          pptest(i,k+1) = pptest(i,k) + dm2(i,k)*(w2(i,k)-w1(i,k))*rdt
!       enddo
!    enddo
!
!    do i=is, ie
!       aa(i,km+1)  = dt*2.0*(gm2(i,km)/dz2(i,km)) *(pem(i,km+1)+pp(i,km+1))
!       pe(i,km+1) = pp(i,km+1) -  aa(i,km+1)*(ws(i)-w2(i,km))
!       pptest(i,km+1) = pptest(i,km) + dm2(i,km)*(w2(i,km)-w1(i,km))*rdt
!    enddo
