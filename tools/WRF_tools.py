...
def read_wrf_fields(run_dir, vars = [''], printout=False, filename=None, precip_only=False):
    ...
    for key in variables:
        ...
        if key == 'press':
            dsout['press'] = ds.P.values + ds.PB.values

        if key == 'pert_p':
            dsout['press'] = ds.P.values
        ...
        if printout:
            write_Z_profile(dsout, model='WRF')
...
def generate_ideal_profiles(run_dir, model_type='wrf', filename=None, w_thresh = 5.0, cref_thresh = 45., 
    ...
    if model_type == 'fv3_solo':
        ...
        pert_p  = ds.nhpres_pert.values[:,::-1,:,:]
        pert_pss= ds.nhpres_pert_ss.values[:,::-1,:,:]
