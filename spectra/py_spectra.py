...
def get_spectra2dr1(fld):
    """
    Returns 1D radial power spectra from a 2D field.  There are several caveats.  First, 
    the shape of the fld array must be square.  If it is not square, then it is made
    square by using the smallest dimension - one way or another the 2D spectrum can only 
    ...
    if nx != ny:
        ...
        print("get_spectra2dr1: can only analyze process same wavenumbers in X & Y, nx: %d  ny: %d\n" %(nx,ny))
        print("get_spectra2dr1: will sample a square domain using nx/2, ny/2 center point\n")
...
def plot_spectra2d(fld, title = None, ax = None, PScolor='k', **kwargs):
    ...
    kvals, Abins, waven = get_spectra2dr1(fld)
    ...
    if 'loglog' in kwargs:
        ...
        for w in [6.0, 8.0, 10.0, 12.0, 16.0]:

            axes[0].annotate(r"%d" % int(w), xy = (2.0/w-0.01, -0.035), xycoords='axes fraction', color='k',fontsize=12,weight='bold')
            axes[0].axvline(x = 2.0/w, color = 'grey', label = 'axvline - full height')
