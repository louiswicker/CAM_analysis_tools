...
parser.add_argument("var",type=str,help='Evaluated Variable (u,v,w,KE)')
parser.add_argument("lev",type=int,help='Index of vertical level to evaluation')
...
for tindex, time in enumerate(times):
   #--- Pick From The Available Model Output Select Variables (u,v,w,KE)
   if model == 'fv3':

      dumpfile  = nc4.Dataset('/work/wicker/ECONUS/2022060700_RRFS_ECONUS.nc')
      print("Reading from RRFS")
      if arguments.var == 'w':
         varname = 'w_interp'
         data = dumpfile.variables[varname][tindex, lev, :,:].astype("float64")
         data = data[None,None,:,:]
         ...
         data = dumpfile.variables[varname][tindex, lev, :,:].astype("float64")
         data = data[None,None,:,:]
      elif arguments.var in ['KE']:
         u = dumpfile.variables['u_interp'][tindex, lev,:,:].astype("float64")
         v = dumpfile.variables['v_interp'][tindex, lev,:,:].astype("float64")
         data = 0.5*( (u**2.) + (v**2.) )
         data = data[None,None,:,:]

   elif model == 'hrrr':
      dumpfile  = nc4.Dataset('/work/wicker/ECONUS/2022060700_HRRR_ECONUS.nc')
      if arguments.var == 'w':
         varname = 'w_interp'
         data = dumpfile.variables[varname][tindex, lev, :,:].astype("float64")
         data = data[None,None,:,:]
