...
def compute_thetae(profile):
    """
        Returns a field of THETAE in shape of input vectors from profile
    """
    
    sphum = (profile['qv'] * (1.0 + profile['qv']) * 1000.).transpose()
    Tc    = (profile['temp']-273.16).transpose()
    pres  = (profile['pres'] / 100.).transpose()
    
    thetae = np.zeros_like(Tc)
    
    for n in np.arange(Tc.shape[0]):
                
        Tdc = dewpoint_from_specific_humidity(pres[n]*units.hPa, 
                                                     Tc[n]*units.degC, 
                                                     sphum[n]*units('g/kg'))

        thetae[n] = equivalent_potential_temperature(pres[n]*units.hPa, 
                                                            Tc[n]*units.degC, 
                                                            Tdc).magnitude          
    return thetae.transpose()
