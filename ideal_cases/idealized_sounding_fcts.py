...
def getdcape(p, T, qv, idx, pinc=-10.0, returnB=False, returnTHV=False, returnQV=False):
    ...
    -------
    dcape_tot : float
        Total downdraft convective available potential energy (J / kg)
    dcape_max : float
        Same as dcape_tot, but only include negative B contributions (J / kg)
    B : array
        Buoyancy profile following the lifted parcel (m / s^2)
...
def mccaul_weisman(z, E=2000.0, m=2.2, H=12500.0, z_trop=12000.0, RH_min=0.1, p_sfc=1e5,
    ...
    z_mid = (z_trop + pbl_z)/2
    depth = H-pbl_z
    a = E / ((depth**3)/3 - depth**2 * z_mid + pbl_z * H * depth)
    print('first pass')
    for i in range(pbl_top_ind-np.floor(250/dz).astype(int), z.size):
    #for i in range(pbl_top_ind, z.size):
        ...
        if (z[i] <= z_trop):
            ...
            #B = (E * ((m / H) ** 2) * (z[i] - pbl_z) * np.exp(-(m / H)**2 * (z[i] - pbl_z)**2))
            B = a * (z[i]-pbl_z)*(z[i]-H)
    ...
    while (np.abs(E - E_t) > 0.5):
        ...
        #for i in range(pbl_top_ind, z.size):
            ...
            if (z[i] > pbl_z) and (z[i] <= z_trop):
                ...
                #B = ratio * E * ((m / H) ** 2) * (z[i] - pbl_z) * np.exp(-(m / H)**2 * (z[i] - pbl_z)**2)
                B = ratio * a * (z[i]-pbl_z)*(z[i]-H)
