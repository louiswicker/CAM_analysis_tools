C---------------------------------------------------------------
C
C       3D ARRAY DRIVER:  FILTERING is ISOTROPIC IN ALL DIR
...
C Passed variables
        ...
        REAL*8,    INTENT(IN)  :: XY3D(NX,NY,NZ)
        REAL*8,    INTENT(OUT) :: RESULT3D(NX,NY,NZ)
...
C          DO I = 1,NX
C          DO K = 1,NZ  
C            XY(K) = RESULT3D(I,J,K)
C          ENDDO
C          CALL RAYMOND1D_LOWPASS(XY,RESULT,EPS,NZ)
C          DO K = 1,NZ
C            RESULT3D(I,J,K) = RESULT(K)
C          ENDDO
C          ENDDO
C          ENDDO
C        ENDIF
...
C---------------------------------------------------------------
C
C       3D ARRAY DRIVER WHICH USES DIFFERENT COEFFICIENTS FOR
C       HORIZONTAL AND VERTICAL FILTERING.
C       
C       EPS(1) = HORIZONTAL COEFF, 0 ==> no filtering
C       EPS(2) = VERTICAL COEFF,   0 ==> no filtering
C
...
C---------------------------------------------------------------
C
C       2D ARRAY DRIVER:  FILTERING IS ISOTROPIC IN BOTH DIR
